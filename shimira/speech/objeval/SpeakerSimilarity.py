...
def calc_speaker_similarity_from_waveform(
    wave_a: np.ndarray, wave_b: np.ndarray, encoder: VoiceEncoderModel
) -> float:
    ...
    if len(wave_a.shape) != 2 or wave_a.shape[0] != 1:
        raise ValueError(
            f"入力波形Aのshapeが変です: [1, T] が予期されましたが、{wave_a.shape} が入力されました"
        )
    if len(wave_b.shape) != 2 or wave_b.shape[0] != 1:
        raise ValueError(
            f"入力波形Bのshapeが変です: [1, T] が予期されましたが、{wave_b.shape} が入力されました"
        )
...
def calc_speaker_similarity_from_file(
    fpath_a: str | pathlib.Path, fpath_b: str | pathlib.Path, encoder: VoiceEncoderModel
) -> float:
